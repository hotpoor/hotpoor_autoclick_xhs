...
def x4facesDior(num):
    ...
    # resize logo
    imgLogo = imread('/Users/ryan/PycharmProjects/detect_mouth/venv/pics/logo/dior.jpg',cv2.IMREAD_UNCHANGED)
    h, w = img0.shape[:2]
    dim = (w, int(h/3))
    imgLogo = cv2.resize(imgLogo, dim, interpolation = cv2.INTER_AREA)
    img9 = np.concatenate([img0,img1,imgLogo], 0)# 竖着拼起来
    #resize final pic
    ...
    imwrite("pics/final/2x2d%s.jpg"% num, img9)
